...
export async function listCampaigns(userId: number, scope: 'mine' | 'all' = 'mine') {
  if (scope === 'all') {
    return listAllCampaigns(db);
  }
...
export async function createCampaign(args: {
  ...
  return db.transaction(async (trx) => {
    ...
    const existingRecipients = await findRecipientsByEmails(trx, requestedEmails);
    ...
    if (missingEmails.length > 0) {
      throw validationError('Some recipients do not exist in the system', {
...
export async function scheduleCampaignAt(args: { userId: number; campaignId: number; scheduledAt: Date }) {
  ...
  if (hasConflict) {
    throw conflict('Another campaign is already scheduled at this time');
  }
